...
def run_simulator(sim: sim_utils.SimulationContext, entities: dict[str, Articulation], origins: torch.Tensor):
    ...
    with open("joint_positions_viscous_friction.csv", mode="w", newline="") as f:
        writer = csv.writer(f)
        while simulation_app.is_running():
            robot.set_joint_position_target(torch.zeros(1, 12) + math.pi/4)
            joint_pos = robot.data.joint_pos[0]
            joint_vel = robot.data.joint_vel[0]

            joint_vel = joint_vel.tolist()
            writer.writerow(joint_pos.tolist())
